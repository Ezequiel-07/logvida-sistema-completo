...
export function TimelineClient({ orderId }: { orderId: string }) {
  ...
  useEffect(() => {
    if (!isLoaded || !order || directions) return;
      ...
      directionsService.route({
        ...
        if (status === 'OK' && result) {
          setDirections(result);
           ...
           console.warn(`Admin map route calculation failed: ${status}`);
        }
      });
    }
  ...
  useEffect(() => {
    ...
    const unsubscribeOrder = onSnapshot(
      ...
      async (docSnap) => {
        if (docSnap.exists()) {
          const loadedOrder = sanitizeOrderFromFirestore(docSnap, "PublicTimeline");
            ...
            if (!unsubscribeDriver) {
              ...
              unsubscribeDriver = onSnapshot(driverLocationRef, (driverSnap) => {
                if (driverSnap.exists()) {
                  ...
                  setDriverLocation({ lat: data.latitude, lng: data.longitude, heading: data.heading ?? null });
                }
              });
            }
          } else {
  ...
  const handleCopyTrackingLink = () => {
    if (!order) return;
    const trackingUrl = `${originUrl}/track/${order.id}`;
    navigator.clipboard.writeText(trackingUrl);
    toast({
        title: "Link Copiado!",
        description: "O link de rastreamento foi copiado para sua área de transferência.",
    });
  };
  ...
  return (
    <>
      <div className="space-y-6 max-w-3xl mx-auto">
        <div className="relative h-64 md:h-80 w-full rounded-2xl overflow-hidden shadow-lg border">
        ...
        <Card className="shadow-lg -mt-16 z-10 relative">
          <CardHeader>
             <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
              <div>
                <p className="text-sm font-semibold text-primary">ACOMPANHAMENTO DA ORDEM DE SERVIÇO</p>
          ...
          <CardContent className="space-y-4">
           ...
           <CardFooter className="flex justify-center flex-wrap gap-2 bg-background p-4 mt-6 border-t">
              <Button variant="outline" onClick={handleCopyTrackingLink}>
                <Icons.copy className="mr-2 h-4 w-4"/>
                Copiar Link de Rastreio
              </Button>
              {(order.executionStatus === 'pending' || order.executionStatus === 'in-progress') && (
                <Button variant="outline" onClick={() => router.push(`/orders/${order.id}/edit`)}>
                  <Icons.edit className="mr-2 h-4 w-4"/>
                  Editar Pedido
                </Button>
              )}
              {(order.executionStatus === 'completed' || order.executionStatus === 'cancelled') && (
                <Button variant="default" onClick={() => router.push(`/routes/${order.id}/report`)}>
